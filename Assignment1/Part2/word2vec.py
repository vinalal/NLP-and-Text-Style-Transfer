...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    naive = softmax(np.dot(outsideVectors,centerWordVec))
    loss = -np.log(naive[outsideWordIdx])

    gradCenterVec = -outsideVectors[outsideWordIdx] + np.dot(naive,outsideVectors)

    gradOutsideVecs = np.dot(naive.reshape(-1, 1), centerWordVec.reshape(1, -1)) # when w != o
    gradOutsideVecs[outsideWordIdx] = np.dot((naive[outsideWordIdx].reshape(-1, 1) - 1.0), centerWordVec.reshape(1, -1))
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    pOW = sigmoid(scores[0])
    pNW = sigmoid(-scores[1:])
    loss = -np.log(pOW) - np.sum(np.log(pNW))
