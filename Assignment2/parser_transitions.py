...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        first_batch = unfinished_parses[:batch_size]
        transitions = model.predict(first_batch)
        
        temp = [first_batch[i] for i in range(len(transitions)) if not first_batch[i].parse_step(transitions[i]) and not (len(first_batch[i].stack) == 1 and len(first_batch[i].buffer)==0)]
            
        unfinished_parses = temp + unfinished_parses[batch_size:]
